...
NachOSThread::NachOSThread(char* threadName)
{
	...
	if (algo_num == PREEMPTIVE_PRIORITY) {
		threadPriority += BASE_PRIORITY;
	}
	else if (algo_num == NON_PREEMPTIVE_SJF)
		threadPriority = SJF_INITIAL;
	if (pid == 0)
		threadPriority = BASE_PRIORITY;
...
NachOSThread::NachOSThread(char* threadName, int priority_val)
{
    int i;
	...
	threadPriority = priority_val;
	if (algo_num == PREEMPTIVE_PRIORITY) {
		threadPriority += BASE_PRIORITY;
		//printf("***************Thread Priority: %d*************\n", threadPriority);
	}
	else if (algo_num == NON_PREEMPTIVE_SJF)
		threadPriority = SJF_INITIAL;			// standard  initialization
...
NachOSThread::Exit (bool terminateSim, int exitcode)
{
	...
	if (status == RUNNING) {
		...
		if (run_time > 0) {
			// a non zero cpu burst
			stats->num_cpu_bursts++;
			// update max and min cpu_burst_time if required
			if (run_time > stats->max_cpu_burst)
				stats->max_cpu_burst = run_time;
			if (run_time < stats->min_cpu_burst)
				stats->min_cpu_burst = run_time;
			// Update the priorities now
			if (algo_num == PREEMPTIVE_PRIORITY) {
				// Update the usage & priority of all threads
				scheduler->UpdateUsageAndPriority(run_time);
			}
			if (algo_num == NON_PREEMPTIVE_SJF) {
				stats->burst_estimate_error += abs(stats->totalTicks - cpu_burst_start - this->threadPriority);
				this->threadPriority = (int)(ALPHA * run_time + (1 - ALPHA) * this->threadPriority);
			}
		}
	}
	finishTime[currentThread->GetPID()] = stats->totalTicks;
	//this->finishTime = stats->totalTicks;
	//printf("%d---->\n", stats->totalTicks);
	//printf("Setting finish time = %d at %d......\n", stats->totalTicks, currentThread->GetPID());
    status = BLOCKED;
...
NachOSThread::YieldCPU ()
{
	...
	// for unix baseed scheduling, the thread just finishing execution
	// is also eligible for scheduling next, so add it to readyqueue
	if (algo_num == PREEMPTIVE_PRIORITY) {
		scheduler->MoveThreadToReadyQueue(this);
	}
	// now find the next thread to be scheduled
    nextThread = scheduler->SelectNextReadyThread();

    if (nextThread != NULL) {
		// already added before in case of unix scheduling
		if (algo_num != PREEMPTIVE_PRIORITY)
			scheduler->MoveThreadToReadyQueue(this);
...
NachOSThread::PutThreadToSleep ()
{
	...
	if (status == RUNNING) {
		...
		if (run_time > 0) {
			stats->num_cpu_bursts++;
			//update max and min cpu_busy_time
			if (run_time > stats->max_cpu_burst)
				stats->max_cpu_burst = run_time;
			if (run_time < stats->min_cpu_burst) {
				stats->min_cpu_burst = run_time;
...
// Due to some problems with making finishTime public,
// it was declared as private
// TODO:: Make as many variables private as possible for better control
//int NachOSThread::getFinishTime() {
	//printf("---->  returning finish time of [pid: %d] : %d\n", GetPID(), finishTime);
	//return finishTime;
//}
//void NachOSThread::setFinishTime(int t) {
	//finishTime = t;
	//printf("Set finish Time called for [pid: %d]\n", currentThread->GetPID());
	//printf("----->%d...\n", threadArray[1]->getFinishTime());
//}

int NachOSThread::get_wait_time_start() {
	return wait_start_time;
}

void NachOSThread::set_wait_time_start(int t) {
	wait_start_time = t;
}
