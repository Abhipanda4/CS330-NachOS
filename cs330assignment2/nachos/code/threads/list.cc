...
//-----------------------------------------------------------------------
// List::getMaxPriorityThread
// returns the maximum priority thread and removes it
// from the ReadyQueue
// => returns the thread with the lowest priority_val
//-----------------------------------------------------------------------

void* List::getMaxPriorityThread() {
	if (IsEmpty())
		return NULL;

	int min_till_now = 1000000;
	int curr_priority;
	ListElement* prev = NULL;
	ListElement* max_priority_thread;			//Thread with lowest priority_val
	ListElement* prev_to_max_priority_thread;	//Previous to highest priority  thread in ReadyQueue
	int len = 0;

	for (ListElement *ptr = first; ptr != NULL; ptr = ptr->next) {
		curr_priority = ((NachOSThread*)(ptr->item))->threadPriority;
		if (curr_priority < min_till_now) {
			max_priority_thread = ptr;
			prev_to_max_priority_thread = prev;
			min_till_now = curr_priority;
		}
		//printf("----------> %d \n", curr_priority);
	...
	//printf("%d----%d\n", len, min_till_now);
	if (max_priority_thread == first) {
		// first element is of highest priority
		first = first->next;
		//printf("%d --> Mark1....\n", len);
		if (first == NULL) {
			// The ReadyQueue only had 1 entry
			// printf("Mark2....\n");
			last = NULL;
		}
	} else if (max_priority_thread == last) {
		// last element is of highest priority
		// Sequence cannot be interchanged
		// o/w, max_priority_thread would be lost
		prev_to_max_priority_thread->next = NULL;
		last = prev_to_max_priority_thread;
	} else {
		// highest priority thread is somewhere in between
		prev_to_max_priority_thread->next = max_priority_thread->next;
	}
	void* thing = max_priority_thread->item;
	delete max_priority_thread;
	return thing;
}
