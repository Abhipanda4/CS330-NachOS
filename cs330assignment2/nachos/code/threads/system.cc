...
int finishTime[MAX_THREAD_COUNT];

TimeSortedWaitQueue *sleepQueueHead;    // Needed to implement SC_Sleep

unsigned int algo_num = 1;			//defaults to FCFS algorithm
...
TimerInterruptHandler(int dummy)
{
    ...
    if (interrupt->getStatus() != IdleMode) {
		...
		if (algo_num == NON_PREEMPTIVE_SJF || algo_num == NON_PREEMPTIVE_FCFS) {
			/*DO NOT CALL YieldOnreturn() here*/
		} else {
			/* preemptive scheduling */
			if (stats->totalTicks - cpu_burst_start >= Q3) {
