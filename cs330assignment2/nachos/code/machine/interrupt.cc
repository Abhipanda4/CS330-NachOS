...
Interrupt::Halt()
{
	...
	int i;
	// start from thread 1 since main thread is not included
	int total_time = 0;
	for (i = 1; i < thread_index; i++) {
		int tmp = finishTime[i];
		if (tmp > stats->max_thread_completion)
			stats->max_thread_completion = tmp;
		if (tmp < stats->min_thread_completion)
			stats->min_thread_completion = tmp;
		total_time += tmp;
	}
	stats->avg_thread_completion = (float)total_time / (i - 1);
	for (i = 1; i < thread_index; i++) {
		int tmp = (stats->avg_thread_completion - finishTime[i]);
		stats->var_thread_completion += tmp * tmp;
	}
	stats->var_thread_completion = (float)stats->var_thread_completion / (i - 1);
	stats->Print();
