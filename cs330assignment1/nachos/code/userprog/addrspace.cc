...
ProcessAddressSpace::ProcessAddressSpace(OpenFile *executable)
{
	...
	if (noffH.code.size > 0) {
		...
		executable->ReadAt(&((machine->mainMemory + offset  * PageSize)[noffH.code.virtualAddr]),
				noffH.code.size, noffH.code.inFileAddr);
	}
	if (noffH.initData.size > 0) {
		DEBUG('a', "Initializing data segment, at 0x%x, size %d\n", 
				noffH.initData.virtualAddr, noffH.initData.size);
		executable->ReadAt(&((machine->mainMemory + offset  * PageSize)[noffH.initData.virtualAddr]),
