...
class NachOSThread {
	...
	// Struct for the linked list of children
	// Struct for doubly linked list of sleeping threads
	struct sleepList {
		NachOSThread* thread;
		int wakeUpTick;
		sleepList* next;
	...
	private:
	// For the process children and parents
	static int countPID;
	static NachOSThread* procHead;


	static sleepList* sleepHead;

	public:
	static int totalActiveThreads;
	static void addToSleepQueue(int w, NachOSThread* t);
	static void popFromSleepQueue(int c);
	childList* children;
	NachOSThread* parent;
	...
	// boolean to set if parent waits for child
	bool isParentWaiting;
	childList* findChild(int pid);
