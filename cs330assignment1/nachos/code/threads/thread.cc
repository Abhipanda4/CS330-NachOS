...
int NachOSThread::PIDCounter = 1;
int NachOSThread::totalActiveThreads = 0;

// Initialize head of "proc table"
NachOSThread* NachOSThread::procHead = NULL;

// Initialize sleepList head
NachOSThread::sleepList* NachOSThread::sleepHead = NULL;

// Add thread to sleepQueue and have it sorted
void NachOSThread::addToSleepQueue(int w, NachOSThread* t) {
	if (sleepHead == NULL) {
		sleepList* s = new sleepList();
		s->next = NULL;
		s->prev = NULL;
		s->wakeUpTick = w;
		s->thread = t;
	...
	sleepList* iter = sleepHead;
	while(iter->next != NULL && w >= iter->wakeUpTick) {
		iter = iter->next;
	}
	if(iter->next == NULL) {
		sleepList* s = new sleepList();
...
// Pop threads when they need be woken
void NachOSThread::popFromSleepQueue(int c) {
	sleepList* iter = sleepHead;
	if (iter == NULL) {
		return;
	}
	while(true) {
		if(iter->wakeUpTick <= c) {

			IntStatus oldLevel = interrupt->SetLevel(IntOff);
			scheduler->MoveThreadToReadyQueue(iter->thread);
...
NachOSThread::NachOSThread(char* threadName)
{
	...
	stateRestored = true;
#endif
	pid = NachOSThread::PIDCounter;
	NachOSThread::PIDCounter += 1;
	if (currentThread == NULL) {
		// Current thread is the first, set as global parent
		NachOSThread::procHead = this;
		ppid = -1;
		parent = NULL;
		children = NULL;
	} else {
		...
		if (currentThread->children == NULL) {
			...
			c->childPID = pid;
			c->next = NULL;
			currentThread->children = c;
		} else {
			childList* iter = currentThread->children;
...
#endif
childList* NachOSThread::findChild(int pid) {
	bool flag = false;
	childList* iter = this->children;
	while (iter) {
		if (iter->childPID == pid) {
			flag = true;
			break;
		}
	}
