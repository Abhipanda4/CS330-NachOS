...
LaunchUserProcess(char *filename)
{
    ...
    space = new ProcessAddressSpace(executable, filename, currentThread->GetPID());
...
ReadInputAndFork (char *filename)
{
   ...
   for (i=0; i<batchSize; i++) {
      ...
      child->space = new ProcessAddressSpace (inFile, batchProcesses[i], currentThread->GetPID());
      delete inFile;
